...
std::vector<float> design_filter(int interpolation, int decimation, float fractional_bw = 0.4)
{
	...
	//## Syntax for Gnuradio 3.10.11.0

	taps = gr::filter::firdes::low_pass(interpolation, interpolation, mid_transition_band,
										trans_width, gr::fft::window::win_type::WIN_KAISER, beta);
	//## Syntax for Gnuradio 3.7.14.4
...
po::options_description set_program_args(std::shared_ptr<UserArgs> user_args)
{
	...
	desc.add_options()
	("help",         "help message")
	("name",         po::value<std::string>(&(user_args->device_addr))->default_value("fm_broadcast_receiver"), "a descriptive name for the program")
	("station",      po::value<double>(&(user_args->station))->default_value(99.9), "the fm station you wish to tune into")
	("volume",       po::value<double>(&(user_args->volume))->default_value(5.0), "should be an integer between 0 and 10")
	("sample-rate",  po::value<double>(&(user_args->sample_rate))->default_value(1e6), "rate of incoming samples")
	("output-rate",  po::value<double>(&(user_args->output_rate))->default_value(43e3), "rate at which samples will be fed to the computer's audio card")
	("deviation",    po::value<double>(&(user_args->deviation))->default_value(75e3), "fm broadcast deviation")
	("device",       po::value<std::string>(&(user_args->device_addr))->default_value(""), "multi uhd device address args")
	("channel",      po::value<size_t>(&(user_args->channel))->default_value(0), "which channel to use")
	("ref-src",      po::value<std::string>(&(user_args->ref_src))->default_value("internal"), "reference source (internal, external, mimo)")
	("setup",        po::value<double>(&(user_args->setup_time))->default_value(1.0), "seconds of setup time");
	return desc;
}

int setup_usrp_device(uhd::usrp::multi_usrp::sptr usrp_device, std::shared_ptr<UserArgs> user_args, double *actual_sample_rate, double *actual_gain, po::variables_map vm)
{
	// Instantiate Device
	if (not((user_args->ref_src.compare("internal")) == 0 or
			(user_args->ref_src.compare("external")) == 0 or
			(user_args->ref_src.compare("mimo")) == 0)) {
		std::cerr << "Invalid reference source. Reference source should be one of (internal, external, mimo)" << std::endl;
	return ~0;
		}

		std::cout << boost::format("Instantiating the usrp usrp_device device with address: %s...") % user_args->device_addr << std::endl;
		...
		std::cout << boost::format("Setting RX Rate: %5.2f Msps...") % (desired_sample_rate / 1e6) << std::endl;
		...
		*actual_sample_rate = usrp_device->get_rx_rate(user_args->channel);
		std::cout << boost::format("Actual  RX Rate: %f Msps...") % (*actual_sample_rate / 1e6)	<< std::endl;
...
int UHD_SAFE_MAIN(int argc,char *argv[]){
	...
	//Demand exit signal give you all his data before he is allowed to leave
