 ...
 * stream_cmd_t in UHD's types.cpp, since stream_cmd_t isn't used here)
 **********************************************************************/
    ...
    switch (this->error_code) {
        ...
        default:
            errstr = std::format(
                ...
                static_cast<std::underlying_type_t<decltype(error_code)>>(error_code)
            );
    }
...
std::string async_metadata_t::strevent() const
{
    ...
    switch (event_code) {
        ...
        case EVENT_CODE_USER_PAYLOAD:
            eventstr = std::format(
                "EVENT_CODE_USER_PAYLOAD 0x{:08x} 0x{:08x} 0x{:08x} 0x{:08x}",
                user_payload[0], user_payload[1], user_payload[2], user_payload[3]
            );
            break;
        default:
            eventstr = std::format(
                "Unknown event code: 0x{:x}",
                static_cast<std::underlying_type_t<decltype(event_code)>>(event_code)
            );
    }
