...
void send_packet_handler_mmsg::lock_channel_streaming(size_t channel_num) {
    ...
    if (r == -1) {
        ...
        if (err == EWOULDBLOCK) {
            throw std::runtime_error("Another instance of UHD is currently using channel " + std::to_string(channel_num) + ".");
        } else {
            throw std::runtime_error("flock failed to lock streaming for channel " + std::to_string(channel_num) + " with error: " + std::string(strerror(err)));
        }
    }
}
...
void send_packet_handler_mmsg::setup_converter(const std::string& cpu_format, const std::string& wire_format, bool wire_little_endian) {
    // No conversion support: samples are always sent as-is via scatter/gather
    if(!(cpu_format == wire_format && wire_little_endian)) {
        throw std::invalid_argument("cpu_format must equal wire_format and wire_little_endian must be true; sample format conversion is not supported");
    }
}
